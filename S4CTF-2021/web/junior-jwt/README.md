...
we're given this source and we should get the flag by being authorized as admin:
...
we can see that it doesn't set the `$algo` parameter when decoding user provided jwt string and we can use other signing methods...

using 'HS256' would set the key to hardcoded public key in the code...

with a little modification of the index code we can generate ourselves a valid jwt token for admin role...
...
-----END PUBLIC KEY-----';
        ...
        echo JWT::encode($data, $publikKey, 'HS256');
    }
